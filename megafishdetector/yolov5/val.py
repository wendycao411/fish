...
def run(
    ...
    if training:  # called by train.py
        ...
        heatmap_fusion = str(getattr(model, "heatmap_fusion", heatmap_fusion)).lower()
    ...
    if not training:
        ...
        with contextlib.suppress(Exception):
            warmup_ch = int(getattr(model.model, "yaml", {}).get("ch", warmup_ch))
        model.warmup(imgsz=(1 if pt else batch_size, warmup_ch, imgsz, imgsz))  # warmup
    ...
    for batch_i, batch in enumerate(pbar):
        ...
        if len(batch) == 5:
            im, targets, paths, shapes, heatmap = batch
        else:
        ...
        with dt[0]:
            ...
            if heatmap is not None:
                heatmap = heatmap.half() if half else heatmap.float()
        ...
        with dt[1]:
            if compute_loss:
                if heatmap is not None and not isinstance(model, DetectMultiBackend):
                    preds, train_out = model(im, heatmap=heatmap)
                else:
                    preds, train_out = model(im)
            else:
                preds, train_out = (
                    model(im, augment=augment, heatmap=heatmap)
                    if heatmap is not None and not isinstance(model, DetectMultiBackend)
                    else model(im, augment=augment),
                    None,
                )
...
def parse_opt():
    ...
    parser.add_argument(
        "--heatmap-fusion", type=str, default="none", choices=["none", "concat", "modulate"], help="heatmap fusion mode"
    )
    ...
    parser.add_argument("--heatmap-glob", type=str, default="", help="optional rglob pattern for heatmaps")
    parser.add_argument(
        "--heatmap-norm", type=str, default="minmax", choices=["minmax", "standardize", "none"], help="heatmap normalization"
    )
    parser.add_argument(
        "--heatmap-missing", type=str, default="error", choices=["error", "zero"], help="missing heatmap behavior"
    )
