...
KMP_DUPLICATE_LIB_OK=TRUE python train.py \
  ...
  --workers 0 \
  --epochs 100 \
  --project fish-heatmap-modulation \
  --name exp01_yolov5m_bs4_w0 \
  --entity wendycao \
  --heatmap-fusion modulate \
  --heatmap-root /Users/wendycao/fish/processed/extracted_frames_200_heatmap_yolo_separate/heatmaps_npy \
  --heatmap-ext .npy \
  --heatmap-modulate-alpha 0.5 \
  --heatmap-modulate-alpha-mode learnable
"""
...
def expand_first_conv_for_heatmap(csd, model_state, init_mode="zero"):
    """Expands first conv weight from 3->4 input channels for concat fusion."""
    for k, v in model_state.items():
        if (
            ...
            and csd[k].ndim == 4
            and csd[k].shape[1] == 3
            and v.shape[0] == csd[k].shape[0]
            and v.shape[2:] == csd[k].shape[2:]
        ):
            w = v.clone()
            w[:, :3] = csd[k]
            if init_mode == "mean":
                w[:, 3:4] = csd[k].mean(dim=1, keepdim=True)
            else:
                w[:, 3:4].zero_()
            csd[k] = w
            LOGGER.info(f"Expanded pretrained first conv to 4 channels at '{k}' (init_mode={init_mode})")
            break
    return csd
...
def train(hyp, opt, device, callbacks):
    ...
    if opt.use_heatmap_prior and opt.heatmap_fusion == "none":
        opt.heatmap_fusion = "modulate"
    use_heatmap = opt.heatmap_fusion in {"concat", "modulate"}
    if use_heatmap and opt.quad:
        LOGGER.warning("WARNING ⚠️ --quad is not supported with heatmap fusion, disabling quad dataloader")
        opt.quad = False
    ...
    if opt.heatmap_fusion == "none":
        opt.heatmap_fusion = str(data_dict.get("heatmap_fusion", "none")).lower()
    if opt.heatmap_root == "":
        opt.heatmap_root = str(data_dict.get("heatmap_dir", ""))
    if opt.heatmap_ext == ".npy":
        opt.heatmap_ext = str(data_dict.get("heatmap_ext", opt.heatmap_ext))
    if opt.heatmap_glob == "":
        opt.heatmap_glob = str(data_dict.get("heatmap_glob", ""))
    if opt.heatmap_norm == "minmax":
        opt.heatmap_norm = str(data_dict.get("heatmap_norm", opt.heatmap_norm))
    if opt.heatmap_missing == "error":
        opt.heatmap_missing = str(data_dict.get("heatmap_missing", opt.heatmap_missing))
    if opt.use_heatmap_prior and opt.heatmap_fusion == "none":
        opt.heatmap_fusion = "modulate"
    use_heatmap = opt.heatmap_fusion in {"concat", "modulate"}
    if use_heatmap and opt.heatmap_root:
        hr = Path(opt.heatmap_root)
        if not hr.is_absolute():
            data_base = Path(data_dict.get("path", "."))
            opt.heatmap_root = str((data_base / hr).resolve())
    if use_heatmap and not opt.heatmap_root:
        LOGGER.info("Heatmap fusion enabled without --heatmap-root: using path-based fallbacks from image locations")
    if use_heatmap:
        LOGGER.info(
            f"Heatmap config: fusion={opt.heatmap_fusion}, root='{opt.heatmap_root}', ext='{opt.heatmap_ext}', "
            f"norm={opt.heatmap_norm}, missing={opt.heatmap_missing}"
        )
    ...
    model_ch = 4 if opt.heatmap_fusion == "concat" else 3
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally
        ckpt = torch_load(weights, map_location="cpu")  # load checkpoint to CPU to avoid CUDA memory leak
        model = Model(cfg or ckpt["model"].yaml, ch=model_ch, nc=nc, anchors=hyp.get("anchors"))  # create
        model.configure_heatmap_fusion(
            mode=opt.heatmap_fusion,
            alpha=opt.heatmap_modulate_alpha,
            alpha_mode=opt.heatmap_modulate_alpha_mode,
        )
        model = model.to(device)
        exclude = ["anchor"] if (cfg or hyp.get("anchors")) and not resume else []  # exclude keys
        csd = ckpt["model"].float().state_dict()  # checkpoint state_dict as FP32
        if opt.heatmap_fusion == "concat":
            csd = expand_first_conv_for_heatmap(csd, model.state_dict(), init_mode="zero")
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect
        model.load_state_dict(csd, strict=False)  # load
        LOGGER.info(f"Transferred {len(csd)}/{len(model.state_dict())} items from {weights}")  # report
    else:
        model = Model(cfg, ch=model_ch, nc=nc, anchors=hyp.get("anchors"))  # create
        model.configure_heatmap_fusion(
            mode=opt.heatmap_fusion,
            alpha=opt.heatmap_modulate_alpha,
            alpha_mode=opt.heatmap_modulate_alpha_mode,
    ...
    train_loader, dataset = create_dataloader(
        ...
        shuffle=True,
        seed=opt.seed,
        heatmap_fusion=opt.heatmap_fusion,
        heatmap_root=opt.heatmap_root,
        heatmap_ext=opt.heatmap_ext,
        heatmap_glob=opt.heatmap_glob,
        heatmap_norm=opt.heatmap_norm,
        heatmap_missing=opt.heatmap_missing,
    )
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, batch in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.heatmap_fusion == "modulate":
                imgs, targets, paths, _, heatmaps = batch
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    if heatmaps is not None:
                        heatmaps = nn.functional.interpolate(heatmaps, size=ns, mode="bilinear", align_corners=False)

            # Forward
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs, heatmap=heatmaps) if heatmaps is not None else model(imgs)  # forward
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "concat", "modulate"],
        help="heatmap fusion mode: none, concat (RGB+H), modulate (FiLM-style feature gating)",
    )
    parser.add_argument("--heatmap-root", type=str, default="", help="root directory for per-frame heatmaps")
    parser.add_argument("--heatmap-ext", type=str, default=".npy", help="heatmap extension (.npy or .png)")
    parser.add_argument("--heatmap-glob", type=str, default="", help="optional rglob pattern, supports {stem},{parent}")
    parser.add_argument(
        "--heatmap-norm",
        type=str,
        default="minmax",
        choices=["minmax", "standardize", "none"],
        help="heatmap normalization strategy",
    )
    parser.add_argument(
        "--heatmap-missing",
        type=str,
        default="error",
        choices=["error", "zero"],
        help="behavior when a heatmap file is missing for a sample",
    )
    parser.add_argument("--heatmap-modulate-alpha", type=float, default=0.5, help="initial alpha for modulation")
    parser.add_argument(
        "--heatmap-modulate-alpha-mode",
        type=str,
        default="learnable",
        choices=["learnable", "fixed"],
        help="use learnable or fixed alpha at modulation site",
    )
    # Backward-compatible alias: previous flag enabled modulation mode.
    parser.add_argument("--use-heatmap-prior", action="store_true", help=argparse.SUPPRESS)
