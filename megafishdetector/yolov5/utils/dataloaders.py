...
def create_dataloader(
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(
            ...
            prefix=prefix,
            rank=rank,
            heatmap_fusion=heatmap_fusion,
            heatmap_root=heatmap_root,
            heatmap_ext=heatmap_ext,
            heatmap_glob=heatmap_glob,
            heatmap_norm=heatmap_norm,
            heatmap_missing=heatmap_missing,
        )
    ...
    if dataset.heatmap_fusion == "modulate" and not quad:
        collate_fn = LoadImagesAndLabels.collate_fn_heatmap
    return loader(
        dataset,
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.stride = stride
        self.path = path
        self.heatmap_fusion = str(heatmap_fusion or "none").lower()
        assert self.heatmap_fusion in {"none", "concat", "modulate"}, (
            f"{prefix}Invalid heatmap_fusion '{heatmap_fusion}', expected one of ['none', 'concat', 'modulate']"
        )
        self.use_heatmap = self.heatmap_fusion in {"concat", "modulate"}
        self.heatmap_root = Path(heatmap_root) if heatmap_root else None
        self.heatmap_ext = str(heatmap_ext).lower()
        if self.heatmap_ext and not self.heatmap_ext.startswith("."):
            self.heatmap_ext = f".{self.heatmap_ext}"
        self.heatmap_glob = str(heatmap_glob or "")
        self.heatmap_norm = str(heatmap_norm or "minmax").lower()
        self.heatmap_missing = str(heatmap_missing or "error").lower()
        assert self.heatmap_norm in {"minmax", "standardize", "none"}, (
            f"{prefix}Invalid heatmap_norm '{self.heatmap_norm}', expected one of ['minmax', 'standardize', 'none']"
        )
        assert self.heatmap_missing in {"error", "zero"}, (
            f"{prefix}Invalid heatmap_missing '{self.heatmap_missing}', expected one of ['error', 'zero']"
        )
        self._heatmap_file_cache = {}
        self._heatmap_array_cache = {}
        self._frame_regex = re.compile(r"frame(\d+)", re.IGNORECASE)
        self.albumentations = Albumentations(size=img_size) if augment and not self.use_heatmap else None
        if self.use_heatmap and self.heatmap_root is not None and not self.heatmap_root.exists():
            raise FileNotFoundError(f"{prefix}heatmap root does not exist: {self.heatmap_root}")
    ...
    def __getitem__(self, index):
        ...
        if mosaic := self.mosaic and random.random() < hyp["mosaic"]:
            # Load mosaic
            if self.use_heatmap:
                img, labels, heatmap = self.load_mosaic(index)
            else:
            ...
            if random.random() < hyp["mixup"]:
                if self.use_heatmap:
                    img2, labels2, hm2 = self.load_mosaic(random.choice(self.indices))
                    mix_r = np.random.beta(32.0, 32.0)
                    img = (img * mix_r + img2 * (1 - mix_r)).astype(np.uint8)
                    heatmap = (heatmap * mix_r + hm2 * (1 - mix_r)).astype(np.float32)
            ...
            heatmap = None
            if self.use_heatmap:
                heatmap = self._load_heatmap_for_image(self.im_files[index], h0=h0, w0=w0, h=h, w=w)

            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, ratio, pad = letterbox(img, shape, auto=False, scaleup=self.augment)
            if self.use_heatmap:
                heatmap, _, _ = letterbox(heatmap, shape, color=0, auto=False, scaleup=self.augment)
        ...
        if heatmap is None:
            heatmap = np.zeros((img.shape[1], img.shape[2]), dtype=np.float32)
        heatmap = self._normalize_heatmap(heatmap)
        heatmap = np.ascontiguousarray(heatmap[None, :, :])  # [1,H,W]
        if self.heatmap_fusion == "concat":
            hm_u8 = np.clip(heatmap * 255.0, 0, 255).astype(np.uint8)
            img = np.ascontiguousarray(np.concatenate((img, hm_u8), axis=0))
            return torch.from_numpy(img), labels_out, self.im_files[index], shapes
        return torch.from_numpy(img), labels_out, self.im_files[index], shapes, torch.from_numpy(heatmap)

    def _infer_frame_index(self, im_file):
        stem = Path(im_file).stem
        m = self._frame_regex.search(stem)
        if m:
    ...
    def _resolve_heatmap_file(self, im_file):
        im_path = Path(im_file)
        cached = self._heatmap_file_cache.get(str(im_path), "__missing__")
        if cached != "__missing__":
            return cached

        exts = [self.heatmap_ext] if self.heatmap_ext else [".npy", ".png"]
        candidates = []

        if self.heatmap_root is not None:
            rel = None
            token = f"{os.sep}images{os.sep}"
            if token in str(im_path):
                rel = Path(str(im_path).split(token, 1)[1])
            if rel is None:
                rel = Path(im_path.name)
            for ext in exts:
                candidates.append((self.heatmap_root / rel).with_suffix(ext))
                candidates.append(self.heatmap_root / rel.parent / f"{rel.stem}_heatmap{ext}")
                candidates.append(self.heatmap_root / im_path.parent.name / f"{im_path.stem}{ext}")
                candidates.append(self.heatmap_root / im_path.parent.name / f"{im_path.stem}_heatmap{ext}")
                candidates.append(self.heatmap_root / f"{im_path.stem}{ext}")
                candidates.append(self.heatmap_root / f"{im_path.stem}_heatmap{ext}")

        for ext in exts:
            token = f"{os.sep}images{os.sep}"
            if token in str(im_path):
                hm_path = Path(str(im_path).replace(token, f"{os.sep}heatmaps{os.sep}"))
                candidates.append(hm_path.with_suffix(ext))
                candidates.append(hm_path.parent / f"{hm_path.stem}_heatmap{ext}")
            candidates.append(im_path.parent / f"{im_path.stem}_heatmap{ext}")

        if self.heatmap_glob:
            glob_root = self.heatmap_root if self.heatmap_root is not None else im_path.parent
            pattern = self.heatmap_glob.format(stem=im_path.stem, parent=im_path.parent.name, name=im_path.name)
            candidates.extend(sorted(glob_root.rglob(pattern)))

        out = next((p for p in candidates if p.exists()), None)
        self._heatmap_file_cache[str(im_path)] = out
        return out
    ...
    def _normalize_heatmap(self, heatmap):
        ...
        if self.heatmap_norm == "none":
            return heatmap
        if self.heatmap_norm == "standardize":
            mean = float(heatmap.mean())
            std = float(heatmap.std())
            if std > 1e-6:
                heatmap = (heatmap - mean) / std
                heatmap = np.clip((heatmap + 3.0) / 6.0, 0.0, 1.0)
            else:
    ...
    def _load_heatmap_for_image(self, im_file, h0, w0, h, w):
        hm_file = self._resolve_heatmap_file(im_file)
        if hm_file is None:
            msg = f"Missing heatmap for image: {im_file}"
            if self.heatmap_missing == "error":
                raise FileNotFoundError(msg)
            LOGGER.warning(msg)
            return np.zeros((h, w), dtype=np.float32)
        key = str(hm_file)
        heatmap = None
        if hm_file.suffix.lower() == ".png":
            heatmap = cv2.imread(str(hm_file), cv2.IMREAD_GRAYSCALE)
        elif hm_file.suffix.lower() == ".npy":
            arr = self._heatmap_array_cache.get(key)
            if arr is None:
                arr = np.load(hm_file, mmap_mode="r")
                self._heatmap_array_cache[key] = arr
            if arr.ndim == 2:
                heatmap = np.array(arr, dtype=np.float32, copy=False)
            elif arr.ndim == 3:
                frame_idx = self._infer_frame_index(im_file)
                if frame_idx is None or frame_idx < 0 or frame_idx >= arr.shape[0]:
                    msg = f"Cannot index frame heatmap for image: {im_file} in {hm_file}"
                    if self.heatmap_missing == "error":
                        raise FileNotFoundError(msg)
                    LOGGER.warning(msg)
                    heatmap = np.zeros((h, w), dtype=np.float32)
                else:
                    heatmap = np.array(arr[frame_idx], dtype=np.float32, copy=False)
        if heatmap is None:
            msg = f"Unsupported/invalid heatmap file for image: {im_file} -> {hm_file}"
            if self.heatmap_missing == "error":
                raise FileNotFoundError(msg)
            LOGGER.warning(msg)
            heatmap = np.zeros((h, w), dtype=np.float32)
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            heatmap = self._load_heatmap_for_image(self.im_files[index], h0=h, w0=w, h=h, w=w) if self.use_heatmap else None

            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                if self.use_heatmap:
                    hm4 = np.zeros((s * 2, s * 2), dtype=np.float32)
            ...
            if self.use_heatmap:
                hm4[y1a:y2a, x1a:x2a] = heatmap[y1b:y2b, x1b:x2b]
            padw = x1a - x1b
            padh = y1a - y1b
        ...
        return (img4, labels4, hm4) if self.use_heatmap else (img4, labels4)
    ...
    def collate_fn_heatmap(batch):
        """Batches images, labels, paths, shapes and heatmaps [B,1,H,W]."""
        im, label, path, shapes, heatmap = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i
        return torch.stack(im, 0), torch.cat(label, 0), path, shapes, torch.stack(heatmap, 0)
