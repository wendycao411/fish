...
class HeatmapModulation(nn.Module):
    """FiLM-style feature modulation from a single-channel heatmap."""
    ...
    def forward(self, feat, heatmap):
        if heatmap is None:
            return feat
        h_small = torch.nn.functional.interpolate(heatmap, size=feat.shape[-2:], mode="bilinear", align_corners=False)
        return feat * (1.0 + self.alpha * h_small)
