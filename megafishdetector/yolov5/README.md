...
## Audio Heatmap Fusion (Custom)

This fork supports per-frame audio heatmaps in two modes via `train.py`:

- `--heatmap-fusion concat`: input-level fusion, model input is `RGB + heatmap` (4 channels).
- `--heatmap-fusion modulate`: internal feature modulation `F' = F * (1 + alpha * H_small)`.

Heatmap naming convention:

- Image path: `.../images/<split>/<subdirs>/<frame>.jpg`
- Heatmap path (default): `--heatmap-root/<split>/<subdirs>/<frame><heatmap-ext>`
- Supported heatmap formats: `.npy` (2D per-frame array, or 3D `[num_frames,H,W]`) and grayscale `.png`
- If a heatmap is missing and `--heatmap-missing error` (default), training stops with a clear file error.

Example training commands:

```bash
# A) 4-channel concat (RGB + heatmap)
python train.py --data data/your_dataset.yaml --weights yolov5s.pt \
  --imgsz 640 --batch-size 16 \
  --heatmap-fusion concat --heatmap-root /path/to/heatmaps --heatmap-ext .png

# B) Feature modulation
python train.py --data data/your_dataset.yaml --weights yolov5s.pt \
  --imgsz 640 --batch-size 16 \
  --heatmap-fusion modulate --heatmap-root /path/to/heatmaps --heatmap-ext .png \
  --heatmap-modulate-alpha 0.5 --heatmap-modulate-alpha-mode learnable
```

Sanity checker:

```bash
python tools/sanity_heatmap_fusion.py
```
