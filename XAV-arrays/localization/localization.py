...
def calc_tdoa(
    ...
    for hydrophone_pair in hydrophone_pairs:
        ...
        if TDOA_max_sec:
            # Use closest indices in case exact lags are not present.
            if TDOA_max_samp >= np.max(lag_array):
                SW_start_idx = 0
                SW_stop_idx = len(corr) - 1
            else:
                SW_start_idx = int(
                    np.searchsorted(lag_array, -TDOA_max_samp, side="left")
                )
                SW_stop_idx = int(
                    np.searchsorted(lag_array, TDOA_max_samp, side="right") - 1
                )
                SW_start_idx = max(0, min(SW_start_idx, len(corr) - 1))
                SW_stop_idx = max(SW_start_idx, min(SW_stop_idx, len(corr) - 1))
        else:
            SW_start_idx = 0
            SW_stop_idx = len(corr) - 1
        corr_max_idx = (
