...
def _parse_filename_datetime(file_path):
    try:
        return ecosound.core.tools.filename_to_datetime(file_path)[0]
    except ValueError:
        base = os.path.basename(file_path)
        match = re.search(r"(\d{8}T\d{6}(?:\.\d+)?Z)", base)
        if not match:
            raise
        ts = match.group(1)
        try:
...
def run_detector(infile, channel, config, chunk=None, deployment_file=None):
    ...
    spectro.crop(
        frequency_min=config["SPECTROGRAM"]["fmin_hz"],
    ...
    # Detector
    file_timestamp = _parse_filename_datetime(sound.file_full_path)
    ...
    detections.data["time_min_offset"] = (
        detections.data["time_min_offset"] + time_offset_sec
    )
    detections.data["time_max_offset"] = (
        detections.data["time_max_offset"] + time_offset_sec
    )
    ...
    detections.insert_values(
        operator_name=platform.uname().node,
        audio_file_name=file_name,
        audio_file_dir=file_dir,
        audio_file_extension=file_ext,
        audio_file_start_date=_parse_filename_datetime(sound.file_full_path),
    )
    # print('...done')
